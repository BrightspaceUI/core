...
import { linkGenerator, localizeMarkup } from '../../helpers/localize.js';
...
class Mission extends LocalizeDynamicMixin(LitElement) {
	...
	render() {
		...
		return html`
			...
			<div>7. ${this.localizeHTML('a', { a: true, c: localizeMarkup`<b>bold</b>` })}</div>
			<div>8. ${this.localizeHTML('a', { a: true, c: [localizeMarkup`<br>`] })}</div>
