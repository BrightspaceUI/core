...
export const DropdownContentMixin = superclass => class extends LocalizeCoreElement(RtlMixin(superclass)) {
	...
	_renderContent() {
		...
		if (mobileTrayRightLeft) {
			...
			// default to maximum height for tray (42px margin), ignoring maxHeight attribute
			maxHeightOverride = window.innerHeight - minBackdropHeightMobile;
		...
		} else if (!specialMobileStyle && !this._width) {
			contentWidth = '';
			containerWidth = '';
		} else {
