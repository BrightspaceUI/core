...
export const DropdownContentMixin = superclass => class extends LocalizeCoreElement(RtlMixin(superclass)) {
	...
	constructor() {
		...
		this.__onAncestorResize = this.__onAncestorResize.bind(this);
	...
	*/
	...
	__onAncestorMutation(e) {
		this.__reposition('mutation');
	}

	__onAncestorResize(e) {
		this.__reposition('resize');
	}
	...
	async __openedChanged(newValue) {
		...
		if (newValue) {
			...
			window.addEventListener('scroll', this.__onScroll, { capture: true });

			if (this._ancestorResizeObserver) this._ancestorResizeObserver.disconnect();
			else this._ancestorResizeObserver = new ResizeObserver(this.__onAncestorResize);

			if (this._ancestorMutationObserver) this._ancestorMutationObserver.disconnect();
			else this._ancestorMutationObserver = new MutationObserver(this.__onAncestorMutation);
			const config = { attributes: true, childList: true, subtree: true };

			let elem = getComposedParent(this);
			while (elem && elem.tagName !== 'BODY') {
				if (elem.nodeType === Node.ELEMENT_NODE) this._ancestorResizeObserver.observe(elem);
				if (elem.nodeType === Node.ELEMENT_NODE) this._ancestorMutationObserver.observe(elem, config);
				elem = getComposedParent(elem);
			}

		} else {

			window.removeEventListener('scroll', this.__onScroll, { capture: true });
	...
	async __position(ignoreVertical, contentRect) {
		...
		const footer = this.__getContentBottom(); // rodo: rename
		...
		const adjustPosition = async() => {
			...
			const positionXAdjustment = this._getPositionXAdjustment(targetRect, spaceAround, centerDelta);
			const position = {};

			const isRTL = this.getAttribute('dir') === 'rtl'; // todo: move this up
			...
			if (pointer) {
				...
				if (!isRTL) {
					pointerPosition.left = targetRect.left + ((targetRect.width - pointerRect.width) / 2);
				} else {
					pointerPosition.right = window.innerWidth - targetRect.left - ((targetRect.width - pointerRect.width) / 2);
				}
				if (this.openedAbove) {
					pointerPosition.bottom = window.innerHeight - targetRect.top + 8;
				} else {
					pointerPosition.top = targetRect.top + targetRect.height + this._verticalOffset - 7;
				}
			}
	...
	__reposition(reason) {
		// throttle repositioning for scroll performance
		if (!this.__repositionTicking) {
			requestAnimationFrame(() => {
				console.log('reposition', reason)
				this.__position();
				this.__repositionTicking = false;
			});
		}
		this.__repositionTicking = true;
	}
	...
	_getPositionXAdjustment(targetRect, spaceAround, centerDelta) {

		const contentXAdjustment = centerDelta / 2;
		if (centerDelta <= 0) {
			// slide the content in by half the difference to center it with target
			return contentXAdjustment * -1;
		}
		if (spaceAround.left > contentXAdjustment && spaceAround.right > contentXAdjustment) {
			// slide the content out by half the difference to center it with target
			return contentXAdjustment * -1;
		}
		const isRTL = this.getAttribute('dir') === 'rtl';
		if (!isRTL) {
	...
	_renderContent() {
		const positionStyle = {}; // todo: move down
		if (this._position) {
			for(const positionProp in this._position) {
				positionStyle[positionProp] = `${this._position[positionProp]}px`;
			}
		}
