	...
	async _handleTabsSlotChange(e) {
		...
		if (selectedTab) {
			Promise.all(animPromises).then(async() => {
				if (getFlag('GAUD-9963-dropdown-tabs-not-resizing', false)) await new Promise(resolve => requestAnimationFrame(resolve)); /* TODO: when removing the GAUD-9963 flag, keep the Promise */
				this._updateMeasures();
				this._updateScrollPosition(selectedTab);
			});
		}
