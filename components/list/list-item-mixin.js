...
export const ListItemMixin = superclass => class extends RtlMixin(superclass) {
...
:host {
	...
	render() {
		...
		const classes = {
			...
			'd2l-list-item-flex': label || link || !!this._renderOutside(),
	...
	_renderSelectable() {
		return this.selectable
			? html`<input id="${this._checkBoxId}" class="d2l-input-checkbox" @change="${this._handleCheckboxChange}" type="checkbox" .checked="${this.selected}">`
			: html``;
	}

	_renderBeforeContent() {
		return this._renderSelectable();
	}
