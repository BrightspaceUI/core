...
describe('d2l-filter', () => {
	[
		...
		it(`${test.name} opened`, async() => {
			...
			await expect(elem).to.be.accessible({ ignoredRules: ['aria-required-children', 'aria-required-parent'] }); // d2l-list's grid mode does not apply the grid roles because of lack of iOS support
		});
	});
