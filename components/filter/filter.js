...
class Filter extends FocusMixin(LocalizeCoreElement(LitElement)) {
	...
	_createSetDimension(dimension) {
		...
		if (dimension.minWidth) this._minWidth = Math.min(dimension.minWidth, this._minWidth);
	...
	_handleDimensionDataChange(e) {
		...
		changes.forEach((newValue, prop) => {
			...
			if (prop === 'selected') {
				...
				shouldRecount = true;
				this._activeFiltersSubscribers.updateSubscribers();
				newValue.forEach((field) => this._minWidth = field.minWidth ? Math.min(field.minWidth, this._minWidth) : this._minWidth);
			} else if (prop === 'text') {
				this._activeFiltersSubscribers.updateSubscribers();
			}
		});
