...
class LoadingBackdrop extends PropertyRequiredMixin(LocalizeCoreElement(LitElement)) {
	...
	render() {
		const forcedOffscreenSizelessStyles = OffSCREEN_SIZELESS ? nothing : html`style="width:0px;height:0px"`;
		...
		return html`
			...
			<d2l-offscreen ${forcedOffscreenSizelessStyles} aria-live="polite">${this._ariaContent}</d2l-offscreen>
		`;
	}
